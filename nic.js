...
function prabhasha(){
    ...
    if (!nic){
        ...
        if((nic.length) >= 11){
            document.getElementById("type").innerHTML = "- ⚙ - New Identity Card"

            var year = "prabhasha_new"
        }else{
            document.getElementById("type").innerHTML = "- ⚙ - Old Identity Card"
...
if (date <=  32) {document.getElementById("bd").innerHTML = "- Birthday is on : "+ year + " - 01 - " + (date + 1)  } else {
    ...
    if (date <= 366) {document.getElementById("bd").innerHTML = "- Birthday is on : "+ year + " - 12 - " + (date - 335)}
        ...
        if (year === "prabhasha_new"){


var year = nic.slice(0,4)
year = Number(year)
console.log(year)

//if (year > 40) {year = "19" + year} else {year = "20" + year}

var date = nic.slice(4,7)
date = Number(date)
